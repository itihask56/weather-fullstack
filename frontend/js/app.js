class WeatherApp {
  ...
  async addCity() {
    ...
    try {
      ...
      // Parse error response for suggestions
      if (error.message.includes("not found")) {
        this.showCityNotFoundError(city, error);
      } else {
  ...
  showCityNotFoundError(city, error) {
    // Try to parse suggestions from error response
    let suggestions = [];
    try {
      // If the error contains suggestions, extract them
      const errorData = JSON.parse(error.message);
      suggestions = errorData.suggestions || [];
    } catch (e) {
      // Error message is not JSON, continue without suggestions
    }
    ...
    const notification = document.createElement("div");
    notification.className = "notification error city-not-found";
      ...
      suggestionHtml = `
        <div class="suggestions">
          ...
          ${suggestions
            .map(
              (suggestion) =>
                `<button class="suggestion-btn" onclick="weatherApp.addSuggestedCity('${suggestion}')">${suggestion}</button>`
            )
            .join("")}
        </div>
      `;
    ...
    notification.innerHTML = `
      <div class="error-content">
        <span>City "${city}" not found. Please check the spelling and try again.</span>
        ${suggestionHtml}
      </div>
      <button onclick="this.parentElement.remove()">✕</button>
    `;
    ...
    // Auto-remove after 8 seconds (longer for suggestions)
    setTimeout(() => {
      if (notification.parentElement) {
        notification.remove();
      }
